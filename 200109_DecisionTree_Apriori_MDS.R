...
# We have N objects measured on p numeric variables. 
# To represent the distances 
# ***1.Classical MDS-cmdscale( ) 
# ***2.Nonmetric MDS
...
setwd("/Users/kimdongkyu/Desktop/data(f)") ## step1: Read csv
mydata<- read.csv("mydata.csv",header=TRUE, fileEncoding = 'euc-kr')
d <- dist(mydata) # euclidean distances between the rows   ## step2: dist() w/Euclid
d
?dist
fit <- cmdscale(d,eig=TRUE, k=2) # k is the number of dim  ## step3: cmdscale( , , )
fit # view results

# plot solution 
x <- fit$points[,1]                           ## step4-1: fit$points[,1]
y <- fit$points[,2]                           ## step4-2: fit$points[,2]

plot(x, y, xlab="Coordinate 1" +              ## step5-1: plot()
       ylab="Coordinate 2" +       
       main="Metric MDS", type="n")

text(x, y, labels = row.names(mydata) +       ## step5-2: text()
       cex=.7) # Geom size in map = cex argument
#####################################################################################
##### 2.Nonmetric MDS-isoMDS()
...
mydata<-cmdscale(watervoles) # voles
mydata
View(mydata)
fit<-cmdscale(watervoles, eig=TRUE) # vole
fit

plot(mydata, type='n', xlab='Undeveloped', ylab='Developed', main="cmdscale(watervoles)")
segments(-1,0,1,0,lty="dotted")
segments(0,-1,0,1,lty="dotted")
text(mydata, rownames(mydata), cex=0.8, col="orange") # Text Label in PLOTS